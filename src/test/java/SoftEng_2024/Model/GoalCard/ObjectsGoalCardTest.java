...
class ObjectsGoalCardTest {
    ...
    private ObjectsGoalCard testGoal;
    private ObjectsGoalCard testGoal2;
    ...
    void setUp() throws Exception {
        ...
        testGoal = new ObjectsGoalCard(new Angles[]{Angles.INK, Angles.INK}, 2, String.format("Get %s points for each pair of feathers on the board", 2), 0);
        testGoal2 = new ObjectsGoalCard(new Angles[]{Angles.FEATHER, Angles.INK, Angles.SCROLL}, 3, String.format("Get %s points for each triplet of different objects on the board", 3), 0);
    }
    ...
    void calcScore() throws Exception {
        ...
        testCard = new ResourceCard(testFront, false, new Angles[]{Angles.EMPTY, Angles.EMPTY, Angles.EMPTY, Angles.EMPTY, Angles.FUNGI}, 0);
        gameController.getGame().getPlayers().get(0).addCard(testCard);
        ...
        score = testGoal2.calcScore(gameController.getGame().getPlayers().get(0).getPlayerBoard());
        ...
        expectedScore = (scoreCondition) * testGoal2.getPoints();
        assertEquals(expectedScore, score);

        score = testGoal.calcScore(gameController.getGame().getPlayers().get(0).getPlayerBoard());
        expectedScore = (anglesCounter[Angles.INK.ordinal()] / 2) * testGoal.getPoints();
        assertEquals(expectedScore, score);
    }
