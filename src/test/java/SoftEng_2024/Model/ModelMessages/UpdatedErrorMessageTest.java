...
class UpdatedErrorMessageTest {
    ...
    UpdatedErrorMessageTest() throws RemoteException {
    }

    @Test
    void executeMessage() {
        testMessage = new UpdatedErrorMessage(12345, mex);

        testMessage.executeMessage(cliViewClient);
        assertTrue(cliViewClient.getLocalModel().getErrorLog().contains(mex));
    }
}